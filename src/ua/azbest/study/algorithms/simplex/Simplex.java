...
public class Simplex {
    ...
    public static Rational simplex(Rational[][] A, Rational[] b, Rational[] c, Rational[] x) {
        ...
        for (int E = n - 1;;) {
            ...
            for (int i = 0; i < m; i++) {
                if (a[i][E].signum() < 0) {
                    Rational d;
                    if (L < 0 || (d = a[L][n].divides(a[L][E]).minus(a[i][n].divides(a[i][E]))).signum() < 0 || d.signum() == 0
                            && index[L + n] > index[i + n]) {
                        L = i;
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        double[][] a = { { 2, 3 }, { 1, 4 }, { 2, 1.5 } };
        double[] b = { 540, 550, 450 };
        double[] c = { 80, 70 };
